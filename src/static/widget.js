(function () {
  const CONFIG = {
    apiBase: window.location.origin.includes("3002")
  ...
  function init() {
    ...
    async function send() {
      ...
      try {
        ...
        if (data.content) {
          addMessage(data.content, "assistant"); // Map backend 'assistant' to 'bot' style?
          // Actually CSS uses 'bot' for class, but let's stick to 'bot' for class logic
          messages.lastElementChild.classList.replace("assistant", "bot");
          // Wait, my addMessage uses 'role'. If backend returns 'assistant', I should map it.
        } else {
